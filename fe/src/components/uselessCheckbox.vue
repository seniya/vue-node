<template>
  <div style="" class="component-uesless-checkbox" >
    ...
    <div class="checkbox" @mouseover="onHover" @mouseout="offHover">
      <input type="checkbox" @change="onChange" v-model="checked" :disabled="setChecked" />
...
export default {
  ...
  mounted () {
    ...
    // console.log('this.checked mounted  : ', this.checked)
    if (this.checked) this.showTimeline()
  },
  watch: {
    checked (newVal, oldVal) {
      this.$emit('change', newVal)
      // console.log('last Value : ', newVal)
    }
  },
  methods: {
    onClick () {
    ...
    onHover () {
      if (Math.random() > 0.5 && this.count > armLimit) {
        gsap.to(this.bearRef, bearDuration / 2, { y: '35%' })
      }
    },
    offHover () {
      if (!this.checked) {
    ...
    showTimeline () {
      gsap.timeline({
        ...
        .to(
          this.bgRef,
          { duration: checkboxDuration, backgroundColor: '#E1255E' },
          0
        )
...
.component-uesless-checkbox {
  ...
  .bear__wrap {
    ...
    top: 70%;
    left: 35%;
    -webkit-transform: translate(-20%, -55%) rotate(5deg) translate(0, -65%);
            transform: translate(-20%, -55%) rotate(5deg) translate(0, -65%);
